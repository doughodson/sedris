...
int readCSV(FILE* infileSrc, FILE* infileTgt, std::vector<gdDatumCoord>& gdCoord)
{
...
int readCSV(FILE* infile, std::vector<configInfo>& config)
{
   //	declare string buffer
   char buffer[500]{};
   configInfo data{};
...
int readCSV(FILE* infile, srfParams& srf, std::vector<doubleArray3>& inDoubleParam)
{
   //	declare string buffer
   char buffer[500]{};
   doubleArray3 data{};
   ...
   while (!std::feof(infile) && std::strstr(buffer, "END OF HEADER") == NULL) {
      if (buffer[0] != '#' && buffer[0] != '\n') {
         ...
         if (std::strstr(pch, srfParamName[NO_HEIGHT]) != NULL) {
            ...
            case COORDINATES:
            case PROJECTION:
               std::strcpy(srf.type, strtok(NULL, ", '\n'"));
               break;

            case DATUM:
            {
               std::strcpy(srf.datum_name, strtok(NULL, ", '\n'"));
...
double computeTau(const double e, const double phi)
{
   return (std::tan(PI_OVER_4 - (std::fabs(phi) / 2.0)) *
      std::pow((1. + e * std::sin(std::fabs(phi))) / (1.0 - e * std::sin(std::fabs(phi))), e / 2.0));
}
...
{
   FILE* inFile = std::fopen(src_file_name, "r");
   FILE* outFile = std::fopen(tgt_file_name, "r");
...
bool load_file(char* file_name, srfParams& srf, std::vector<doubleArray3>& doubleParam)
{
   FILE* inFile = std::fopen(file_name, "r");
...
bool load_file(char* file_name, std::vector<configInfo>& config)
{
   FILE* inFile = std::fopen(file_name, "r");
