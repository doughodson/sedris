...
)
{
    ...
    else if(!INT_SRF( srf )->isA( BaseSRF::SRF_TYP_M ))
    {
    ...
    else
    {
        ...
        {
            *parameters =
               ((SRF_Mercator *)INT_SRF( srf ))->getSRFParameters();
        }
        catch( Exception e )
        {
            status = e.getCode();
...
)
{
    ...
    else if ((new_srf->methods = new SRM_M_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                    = &cimpl_SRM_M_GetSRFParameters;
    ...
    else
    {
        try
        {
            INT_SRF(srf) = SRF_Mercator::create(orm_code,
                                                rt_code,
                                                *parameters);
