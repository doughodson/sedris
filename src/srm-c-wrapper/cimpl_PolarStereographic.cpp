...
)
{
    ...
    else if(!INT_SRF( srf )->isA( BaseSRF::SRF_TYP_PS ))
    {
    ...
    else
    {
        ...
        {
            *parameters =
               ((SRF_PolarStereographic *)INT_SRF( srf ))->getSRFParameters();
        }
        catch( Exception e )
        {
            status = e.getCode();
...
)
{
    ...
    else if ((new_srf->methods = new SRM_PS_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                    = &cimpl_SRM_PS_GetSRFParameters;
    ...
    else
    {
        try
        {
            INT_SRF(srf) = SRF_PolarStereographic::create(orm_code,
                                                          rt_code,
                                                          *parameters);
