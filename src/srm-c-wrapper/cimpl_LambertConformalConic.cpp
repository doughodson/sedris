...
{
    if (this_object == NULL || parameters == NULL)
    {
    ...
    else if (!INT_SRF( srf )->isA( BaseSRF::SRF_TYP_LCC ))
    {
    ...
    else
    {
        ...
        {
            *parameters =
                ((SRF_LambertConformalConic *)INT_SRF( srf ))->getSRFParameters();
        }
        catch( Exception e )
        {
            status = e.getCode();
...
{
    if (parameters == NULL || new_srf == NULL)
    {
        return SRM_STATCOD_INVALID_INPUT;
    }
    else if ((new_srf->methods = new SRM_LCC_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                    = &cimpl_SRM_LCC_GetSRFParameters;
    ...
    else
    {
        try
        {
            INT_SRF(srf) = SRF_LambertConformalConic::create(orm_code,
                                                             rt_code,
                                                             *parameters);
