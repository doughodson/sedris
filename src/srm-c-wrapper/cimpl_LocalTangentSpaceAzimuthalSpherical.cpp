...
#include "cimpl_LocalTangentSpaceAzimuthalSpherical.hpp"
...
)
{
    ...
    else if (!INT_SRF( srf )->isA( BaseSRF::SRF_TYP_LTSAS ))
    {
    ...
    else
    {
        ...
        {
           *parameters =
               ((SRF_LocalTangentSpaceAzimuthalSpherical *)INT_SRF( srf ))->getSRFParameters();
        }
        catch( Exception e )
        {
            status = e.getCode();
...
)
{
    ...
    else if ((new_srf->methods = new SRM_LTSAS_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                 = &cimpl_SRM_LTSAS_GetSRFParameters;
    ...
    else
    {
        try
        {
            INT_SRF(srf) = SRF_LocalTangentSpaceAzimuthalSpherical::create(orm_code,
                                                                           rt_code,
                                                                           *parameters);
