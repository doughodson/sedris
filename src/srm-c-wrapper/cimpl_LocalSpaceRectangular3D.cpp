...
#include "cimpl_LocalSpaceRectangular3D.hpp"
#include "srm_level_0_int_types.hpp"

extern "C"
{

SRM_Status_Code cimpl_SRM_LSR3D_GetSRFParameters
(
...
)
{
    ...
    else
    {
        ...
        else if (!INT_SRF(srf)->isA(BaseSRF::SRF_TYP_LSR_3D))
        {
        ...
        else
        {
            ...
            {
                *parameters =
                  ((SRF_LocalSpaceRectangular3D *)INT_SRF( srf ))->getSRFParameters();
            }
            catch (Exception e)
            {
                status = e.getCode();
...
)
{
    ...
    else if ((new_srf->methods = new SRM_LSR_3D_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                 = &cimpl_SRM_LSR3D_GetSRFParameters;
    ...
    else
    {
        try
        {
            INT_SRF(srf) = SRF_LocalSpaceRectangular3D::create(orm_code,
                                                               rt_code,
                                                               *parameters);
