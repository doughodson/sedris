...
#include "cimpl_LococentricEuclidean3D.hpp"
...
)
{
    ...
    else if(!INT_SRF( srf )->isA( BaseSRF::SRF_TYP_LCE_3D ))
    {
    ...
    else
    {
        ...
        {
           *parameters =
               ((SRF_LococentricEuclidean3D *)INT_SRF( srf ))->getSRFParameters();
        }
        catch( Exception e )
        {
            status = e.getCode();
...
)
{
    ...
    else if ((new_srf->methods = new SRM_LCE_3D_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                 = &cimpl_SRM_LCE3D_GetSRFParameters;
    ...
    else
    {
        try
        {
           INT_SRF(srf) = SRF_LococentricEuclidean3D::create(orm_code,
                                                             rt_code,
                                                             *parameters);
