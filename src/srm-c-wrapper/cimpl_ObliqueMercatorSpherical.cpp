...
#include "cimpl_ObliqueMercatorSpherical.hpp"
...
)
{
    ...
    else if(!INT_SRF( srf )->isA( BaseSRF::SRF_TYP_OMS ))
    {
    ...
    else
    {
        ...
        {
            *parameters =
               ((SRF_ObliqueMercatorSpherical *)INT_SRF( srf ))->getSRFParameters();
        }
        catch( Exception e )
        {
            status = e.getCode();
...
)
{
    ...
    else if ((new_srf->methods = new SRM_OMS_Method_List)==NULL)
    {
    ...
    new_srf->methods->GetSRFParameters                    = &cimpl_SRM_OM_GetSRFParameters;
    ...
    else
    {
        try
        {
            INT_SRF(srf) = SRF_ObliqueMercatorSpherical::create(orm_code,
                                                                rt_code,
                                                                *parameters);
