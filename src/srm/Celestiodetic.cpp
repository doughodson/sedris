
#include "Celestiodetic.hpp"
#include "TransverseMercator.hpp"
#include "Mercator.hpp"
#include "PolarStereographic.hpp"
#include "LocalTangentSpaceEuclidean.hpp"
#include "LococentricEuclidean3D.hpp"
#include "Exception.hpp"
#include "internal_api/impl_public.h"
#include "impl_cpp_misc.h"
#include "support/srm_validcoord.h"
#include "srfCache.h"

using namespace srm;

// this macro to get to the SRF private data works for all SRFs have the same
// internal data structure
#define GET_IDATA(_orig) ((Impl_SRF*)(((SRF_Celestiodetic*)_orig)->getImpl()))->private_data

SRF_Celestiodetic::SRF_Celestiodetic(void *impl) : BaseSRF_WithEllipsoidalHeight(impl)
{
    _cache = new srfCache();
}

SRF_Celestiodetic::~SRF_Celestiodetic()
{
    if (_cache)
    {
        delete (srfCache*) _cache;
        _cache = 0;
    }
}

/// SRF_Celestiodetic only constructor by ORM and RT codes
SRF_Celestiodetic* SRF_Celestiodetic::create
(
    SRM_ORM_Code orm,
    SRM_RT_Code  rt
)
{
    Impl_Status        status;
    void              *impl_ptr = 0;
    SRF_Celestiodetic *this_srf = 0;

    if (!isLegalORMCode(orm, rt))
        throw Exception(SRM_STATCOD_INVALID_CODE, "create: Invalid ORM code");

    status = Impl_createSRFFromParams(SRM_SRFTCOD_CELESTIODETIC,
                                      orm,
                                      rt,
                                      0,
                                      &impl_ptr);

    if (status == IMPL_VALID)
        this_srf = new SRF_Celestiodetic(impl_ptr);
    else
        processStatus(status, "create");

    /* only return if status == IMPL_VALID */
    return this_srf;
}


/// Returns a 3D coordinate object
Coord3D* SRF_Celestiodetic::createCoordinate3D
(
    SRM_Long_Float coord_comp1,
    SRM_Long_Float coord_comp2,
    SRM_Long_Float coord_comp3
)
{
    return (new Coord3D_Celestiodetic(this, coord_comp1, coord_comp2, coord_comp3));
}


/// Returns a surface coordinate object
CoordSurf* SRF_Celestiodetic::createSurfaceCoordinate
(
    SRM_Long_Float coord_surf_comp1,
    SRM_Long_Float coord_surf_comp2
)
{
    return (new CoordSurf_Celestiodetic(this, coord_surf_comp1, coord_surf_comp2));
}


bool SRF_Celestiodetic::isEqual(const SRF_Celestiodetic &srf) const
{
    return (Impl_compareSRFParameters(_impl, srf._impl) == 0);
}

bool SRF_Celestiodetic::operator== (const SRF_Celestiodetic &rhs) const
{
    return(Impl_compareSRFParameters(_impl, rhs._impl) == 0);
}

SRF_Celestiodetic* SRF_Celestiodetic::makeCopy() const
{
    Impl_Status        status;
    SRF_Celestiodetic *retSRF = 0;
    void              *impl = 0;

    status = Impl_copySRF(_impl, &impl);

    if (status != IMPL_VALID)
        processStatus(status, "makeCopy", SRF, MEM);

    retSRF = new SRF_Celestiodetic(impl);

    return retSRF;
}


SRM_Coordinate_Valid_Region SRF_Celestiodetic::changeCoordinate3DSRF
(
    const Coord3D &src_coord,
          Coord3D &des_coord
)
{
    Impl_Status                 status=IMPL_VALID;
    SRM_Long_Float              desCoord[4], desCoord_tmp[4], srcCoord[4];
    SRM_Long_Float              tmp_vec[3];
    SRM_Coordinate_Valid_Region ret_validity;

    if (des_coord.getSRF() != this)
        throw Exception(SRM_STATCOD_INVALID_TARGET_COORDINATE,
                        "changeCoordinate3DSRF: Target coordinate associated with a different SRF");

    BaseSRF *src_srf = src_coord.getSRF();
    BaseSRF *des_srf = des_coord.getSRF();
    SRF_ClassType src_srf_type = src_srf->getClassType();
    bool doVM = true;

    if (g_fast_mode)
    {
        CD_Cache_Constants *cv_cache = (CD_Cache_Constants*) SRF_CACHE_GET_VALUE(src_srf);

        if (!cv_cache)
        {
            // If the ORMs or RTs don't match we can't do a fast conversion
            if (src_srf->getOrm() != getOrm() || src_srf->getRt() != getRt())
                cv_cache = (CD_Cache_Constants*)1;
            else
            {
                switch (src_srf_type)
                {
                    case SRF_TYP_CC:
                    {
                        if ((cv_cache = new CD_Cache_Constants)!=0)
                        {
                            cv_cache->src_srf_type = src_srf_type;
                            cv_cache->tgt_srf_type = this->getClassType();

                            Impl_ORM_Data *e_constants =
                              Impl_getEconstantsForORMCode(src_srf->getOrm());

                            set_cc_to_cd_constants(e_constants, 0,
                                                   &(cv_cache->cc_cd));

                            free(e_constants);
                        }
                        else
                        {
                            cv_cache = (CD_Cache_Constants*)1;
                        }
                    }
                    break;

                    case SRF_TYP_TM:
                    {
                        if ((cv_cache = new CD_Cache_Constants)!=0)
                        {
                            cv_cache->src_srf_type = src_srf_type;
                            cv_cache->tgt_srf_type = this->getClassType();

                            Impl_ORM_Data *e_constants =
                              Impl_getEconstantsForORMCode(src_srf->getOrm());

                            IMPL_TRANSVERSE_MERCATOR_SRFP tm_srf;
                            SRM_SRF_Code  srf_code;
                            SRM_SRFT_Code template_code;
                            SRM_SRFS_Code_Info srfs_code_info;

                            tm_srf.origin_longitude = ((SRF_TransverseMercator*)src_srf)->get_origin_longitude();
                            tm_srf.origin_latitude = ((SRF_TransverseMercator*)src_srf)->get_origin_latitude();
                            tm_srf.central_scale = ((SRF_TransverseMercator*)src_srf)->get_central_scale();
                            tm_srf.false_easting = ((SRF_TransverseMercator*)src_srf)->get_false_easting();
                            tm_srf.false_northing = ((SRF_TransverseMercator*)src_srf)->get_false_northing();

                            set_tm_to_cd_constants(e_constants, &tm_srf, &(cv_cache->tm_cd.tm_cd));

                            cv_cache->tm_cd.tm_f_e = tm_srf.false_easting;
                            cv_cache->tm_cd.tm_f_n = tm_srf.false_northing;

                            ((SRF_TransverseMercator*)src_srf)->getCodes(srf_code,
                                                                         template_code,
                                                                         srfs_code_info);

                            cv_cache->tm_cd.srf_set = srfs_code_info.srfs_code;

                            free(e_constants);
                        }
                        else
                        {
                            cv_cache = (CD_Cache_Constants*)1;
                        }
                    }
                    break;

                    case SRF_TYP_PS:
                    {
                        if ((cv_cache = new CD_Cache_Constants)!=0)
                        {
                            cv_cache->src_srf_type = src_srf_type;
                            cv_cache->tgt_srf_type = this->getClassType();

                            Impl_ORM_Data *e_constants =
                              Impl_getEconstantsForORMCode(src_srf->getOrm());


                            SRM_SRF_Code  srf_code;
                            SRM_SRFT_Code template_code;
                            SRM_SRFS_Code_Info srfs_code_info;

                            IMPL_POLAR_STEREOGRAPHIC_SRFP ps_srf;

                            ps_srf.origin_longitude = ((SRF_PolarStereographic*)src_srf)->get_origin_longitude();
                            ps_srf.central_scale = ((SRF_PolarStereographic*)src_srf)->get_central_scale();
                            ps_srf.polar_aspect = ((SRF_PolarStereographic*)src_srf)->get_polar_aspect();
                            ps_srf.false_easting = ((SRF_PolarStereographic*)src_srf)->get_false_easting();
                            ps_srf.false_northing = ((SRF_PolarStereographic*)src_srf)->get_false_northing();

                            Impl_set_ps_to_cd_constants(e_constants, &ps_srf,
                                                        &(cv_cache->ps_cd.ps_cd));

                           ((SRF_PolarStereographic*)src_srf)->getCodes(srf_code,
                                                                        template_code,
                                                                        srfs_code_info);

                            // check whether this PS is an UPS.
                            cv_cache->ps_cd.is_ups = (srfs_code_info.srfs_code == SRM_SRFSCOD_UNIVERSAL_POLAR_STEREOGRAPHIC);

                            cv_cache->ps_cd.ps_f_e = ps_srf.false_easting;
                            cv_cache->ps_cd.ps_f_n = ps_srf.false_northing;

                            free(e_constants);
                        }
                        else
                        {
                            cv_cache = (CD_Cache_Constants*)1;
                        }
                    }
                    break;

                    case SRF_TYP_M:
                    {
                        if ((cv_cache = new CD_Cache_Constants)!=0)
                        {
                            cv_cache->src_srf_type = src_srf_type;
                            cv_cache->tgt_srf_type = this->getClassType();

                            Impl_ORM_Data *e_constants =
                              Impl_getEconstantsForORMCode(src_srf->getOrm());

                            IMPL_MERCATOR_SRFP m_srf;

                            m_srf.origin_longitude = ((SRF_Mercator*)src_srf)->get_origin_longitude();
                            m_srf.central_scale = ((SRF_Mercator*)src_srf)->get_central_scale();
                            m_srf.false_easting = ((SRF_Mercator*)src_srf)->get_false_easting();
                            m_srf.false_northing = ((SRF_Mercator*)src_srf)->get_false_northing();

                            set_m_to_cd_constants(e_constants, &m_srf, &(cv_cache->m_cd.m_cd));

                            cv_cache->m_cd.m_f_e = m_srf.false_easting;
                            cv_cache->m_cd.m_f_n = m_srf.false_northing;

                            free(e_constants);
                        }
                        else
                        {
                            cv_cache = (CD_Cache_Constants*)1;
                        }
                    }
                    break;

                    case SRF_TYP_LTSE:
                    {
                        if ((cv_cache = new CD_Cache_Constants)!=0)
                        {
                            cv_cache->src_srf_type = src_srf_type;
                            cv_cache->tgt_srf_type = this->getClassType();

                            Impl_ORM_Data *e_constants =
                              Impl_getEconstantsForORMCode(src_srf->getOrm());

                            set_cc_to_cd_constants(e_constants, 0,
                                                   &(cv_cache->ltse_cd.cc_cd));

                            SRM_SRF_Code  srf_code;
                            SRM_SRFT_Code template_code;
                            SRM_SRFS_Code_Info srfs_code_info;

                            IMPL_LOC_TAN_EUCLIDEAN_SRFP ltse_params;
                            Impl_LTE_SRF_Specific_Data  ltse_spec;

                            ltse_params.geodetic_longitude = ((SRF_LocalTangentSpaceEuclidean*)src_srf)->get_geodetic_longitude();
                            ltse_params.geodetic_latitude = ((SRF_LocalTangentSpaceEuclidean*)src_srf)->get_geodetic_latitude();
                            ltse_params.azimuth = ((SRF_LocalTangentSpaceEuclidean*)src_srf)->get_azimuth();
                            ltse_params.height_offset = ((SRF_LocalTangentSpaceEuclidean*)src_srf)->get_height_offset();

                            Impl_createLTESRFspecificData(e_constants,
                                                          &ltse_params,
                                                          &ltse_spec);

                            set_lte_cc_constants(&ltse_spec,
                                                 &(cv_cache->ltse_cd.ltse_cc));

                            cv_cache->ltse_cd.f_x = ((SRF_LocalTangentSpaceEuclidean*)src_srf)->get_x_false_origin();
                            cv_cache->ltse_cd.f_y = ((SRF_LocalTangentSpaceEuclidean*)src_srf)->get_y_false_origin();

                            ((SRF_LocalTangentSpaceEuclidean*)src_srf)->getCodes(srf_code,
                                                                                 template_code,
                                                                                 srfs_code_info);

                            // check whether this LTSE is a GCS GTRS.
                            cv_cache->ltse_cd.is_gtrs = (srfs_code_info.srfs_code == SRM_SRFSCOD_GTRS_GLOBAL_COORDINATE_SYSTEM);

                            free(e_constants);
                        }
                        else
                        {
                            cv_cache = (CD_Cache_Constants*)1;
                        }
                    }
                    break;

                    case SRF_TYP_LCE_3D:
                    {
                        if ((cv_cache = new CD_Cache_Constants)!=0)
                        {
                            cv_cache->src_srf_type = src_srf_type;
                            cv_cache->tgt_srf_type = this->getClassType();

                            Impl_ORM_Data *e_constants =
                              Impl_getEconstantsForORMCode(src_srf->getOrm());

                            set_cc_to_cd_constants(e_constants, 0,
                                                   &(cv_cache->lce3d_cd.cc_cd));

                            ((SRF_LococentricEuclidean3D*)src_srf)->get_lococentre(cv_cache->lce3d_cd.lce3d_cc.lococentre);
                            ((SRF_LococentricEuclidean3D*)src_srf)->get_primary_axis(cv_cache->lce3d_cd.lce3d_cc.primary_axis);
                            ((SRF_LococentricEuclidean3D*)src_srf)->get_secondary_axis(cv_cache->lce3d_cd.lce3d_cc.secondary_axis);

                            _cross_product
                            (cv_cache->lce3d_cd.lce3d_cc.primary_axis,
                             cv_cache->lce3d_cd.lce3d_cc.secondary_axis,
                             cv_cache->lce3d_cd.lce3d_cc.xprod_rs);

                            free(e_constants);
                        }
                        else
                        {
                            cv_cache = (CD_Cache_Constants*)1;
                        }
                    }
                    break;

                    default:
                         cv_cache = (CD_Cache_Constants*)1;
                         break;
                  }
              }
              SRF_CACHE_ADD_VALUE(src_srf, cv_cache);
          }

          if (SRF_CACHE_CAN_USE(cv_cache))
          {
              doVM = false;

              switch (src_srf_type)
              {
                case SRF_TYP_CC:
                {
                    if ((status = SRM_ChangeCC_CD(&(cv_cache->cc_cd), src_coord.getValues(), desCoord))
                      & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if (_validate_coords && ((status = Impl_validcoord3DCELESTIODETIC(GET_IDATA(des_srf), desCoord))
                                                & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else
                    {
                        SIMPLIFY_STATUS(status);
                    }
                }
                break;

                case SRF_TYP_TM:
                {
                    const SRM_Long_Float *tmp_coord = src_coord.getValues();

                    srcCoord[0] = tmp_coord[0] - cv_cache->tm_cd.tm_f_e;
                    srcCoord[1] = tmp_coord[1] - cv_cache->tm_cd.tm_f_n;
                    srcCoord[2] = tmp_coord[2];

                    if (_validate_coords &&
                        ((status = Impl_validcoord3DTRANSVERSE_MERCATOR_native(GET_IDATA(src_srf), srcCoord))
                                 & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if ((status = SRM_ChangeTM_CD(&(cv_cache->tm_cd.tm_cd), srcCoord, desCoord))
                           & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    // validation for Alabama state plane coordinate system coordinate
                    // is Alabama state plane coordinate system
                    else if (_validate_coords &&
                             (cv_cache->tm_cd.srf_set == SRM_SRFSCOD_ALABAMA_SPCS) &&
                             ((status |= Impl_validcoord3D_ALSP_cd(GET_IDATA(src_srf), desCoord))
                              & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    // validation for Japan plane coordinate system coordinate
                    // is Japan plane coordinate system
                    else if (_validate_coords &&
                             (cv_cache->tm_cd.srf_set == SRM_SRFSCOD_JAPAN_RECTANGULAR_PLANE_CS) &&
                             ((status |= Impl_validcoord3D_JRPS_cd(GET_IDATA(src_srf), desCoord))
                              & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    // validation for UTM coordinate is UTM
                    else if (_validate_coords &&
                             (cv_cache->tm_cd.srf_set == SRM_SRFSCOD_UNIVERSAL_TRANSVERSE_MERCATOR) &&
                             ((status = Impl_validcoord3D_UTMC_cd(GET_IDATA(src_srf), desCoord))
                               & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    // validation for TM coordinate is TM
                    else if (_validate_coords &&
                             !((cv_cache->tm_cd.srf_set == SRM_SRFSCOD_ALABAMA_SPCS) ||
                               (cv_cache->tm_cd.srf_set == SRM_SRFSCOD_JAPAN_RECTANGULAR_PLANE_CS) ||
                               (cv_cache->tm_cd.srf_set == SRM_SRFSCOD_UNIVERSAL_TRANSVERSE_MERCATOR))
                           && ((status = Impl_validcoord3DTRANSVERSE_MERCATOR_cd(GET_IDATA(src_srf), desCoord))
                               & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else
                    {
                        SIMPLIFY_STATUS(status);
                    }
                }
                break;

                case SRF_TYP_PS:
                {
                    const SRM_Long_Float *tmp_coord = src_coord.getValues();

                    srcCoord[0] = tmp_coord[0] - cv_cache->ps_cd.ps_f_e;
                    srcCoord[1] = tmp_coord[1] - cv_cache->ps_cd.ps_f_n;
                    srcCoord[2] = tmp_coord[2];

                    if ((status = Impl_ChangePS_CD(&(cv_cache->ps_cd.ps_cd), srcCoord, desCoord))
                      & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    // validation for UPS coordinate is UPS
                    else if (_validate_coords && cv_cache->ps_cd.is_ups
                           && ((status |= Impl_validcoord3D_UPST_cd(GET_IDATA(src_srf), desCoord_tmp))
                               & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    // validation for PS coordinate is PS
                    else if (_validate_coords && !cv_cache->ps_cd.is_ups
                             && ((status = Impl_validcoord3DPOLAR_STEREOGRAPHIC_cd(GET_IDATA(src_srf), desCoord))
                               & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else
                    {
                        SIMPLIFY_STATUS(status);
                    }
                }
                break;

                case SRF_TYP_M:
                {
                    const SRM_Long_Float *tmp_coord = src_coord.getValues();

                    srcCoord[0] = tmp_coord[0] - cv_cache->m_cd.m_f_e;
                    srcCoord[1] = tmp_coord[1] - cv_cache->m_cd.m_f_n;
                    srcCoord[2] = tmp_coord[2];

                    if ((status = SRM_ChangeM_CD(&(cv_cache->m_cd.m_cd), srcCoord, desCoord))
                        & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if (_validate_coords
                         && ((status = Impl_validcoord3DMERCATOR_cd(GET_IDATA(src_srf), desCoord))
                               & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else
                    {
                        SIMPLIFY_STATUS(status);
                    }
                }
                break;

                case SRF_TYP_LCE_3D:
                {
                    if ((status = SRM_ChangeLCE_CC(&(cv_cache->lce3d_cd.lce3d_cc), src_coord.getValues(), desCoord_tmp))
                      & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if ((status = SRM_ChangeCC_CD(&(cv_cache->lce3d_cd.cc_cd), desCoord_tmp, desCoord))
                           & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if (_validate_coords &&
                           ((status = Impl_validcoord3DCELESTIODETIC(GET_IDATA(des_srf), desCoord))
                            & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else
                    {
                        SIMPLIFY_STATUS(status);
                    }
                }
                break;

                case SRF_TYP_LTSE:
                {
                    const SRM_Long_Float *tmp_coord = src_coord.getValues();

                    srcCoord[0] = tmp_coord[0] - cv_cache->ltse_cd.f_x;
                    srcCoord[1] = tmp_coord[1] - cv_cache->ltse_cd.f_y;
                    srcCoord[2] = tmp_coord[2];
                    srcCoord[3] = 1.0;

                    // only check if it is a GTRS GCS SRF.
                    if (_validate_coords && cv_cache->ltse_cd.is_gtrs
                      && ((status = Impl_validcoord3D_GCSG(GET_IDATA(src_srf), srcCoord))
                          & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if ((status = SRM_ChangeLTE_CC(&(cv_cache->ltse_cd.ltse_cc), srcCoord, desCoord_tmp))
                           & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if ((status = SRM_ChangeCC_CD(&(cv_cache->ltse_cd.cc_cd), desCoord_tmp, desCoord))
                           & IMPL_BADSTAT)
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else if (_validate_coords
                           && ((status = Impl_validcoord3DCELESTIODETIC(GET_IDATA(des_srf), desCoord))
                               & IMPL_BADSTAT))
                    {
                        SIMPLIFY_STATUS(status);
                    }
                    else
                    {
                        SIMPLIFY_STATUS(status);
                    }
                }
                break;
            }
        }
    }

    if (doVM)
    {
        status = Impl_changeCoord3DSRF(((SRF_Celestiodetic*)src_srf)->_impl, src_coord.getValues(),
                                       ((SRF_Celestiodetic*)des_srf)->_impl, desCoord);
    }

    // set the coordinate values if status is "valid"
    if (status == IMPL_VALID || status == IMPL_EXTENDED || status == IMPL_DEFINED)
    {
        memcpy(tmp_vec, desCoord, 3*sizeof(SRM_Long_Float));
        des_coord.setValues(tmp_vec);

        if (status == IMPL_VALID)
            ret_validity = SRM_COORDVALRGN_VALID;
        else if (status == IMPL_EXTENDED)
            ret_validity = SRM_COORDVALRGN_EXTENDED_VALID;
        else if (status == IMPL_DEFINED)
            ret_validity = SRM_COORDVALRGN_DEFINED;
    }
    else
        processStatus(status, "changeCoordinate3DSRF", COORD, MEM);

    return ret_validity;
}


/// Return char* ORM parameter
const char* SRF_Celestiodetic::toString()
{
    static char  ret_string[20];
    static char  tmp_buffer[20];
    Impl_Status  status;
    SRM_ORM_Code orm;
    SRM_RT_Code  rt;

    status = Impl_getORM(this->_impl, &orm);

    if (status != IMPL_VALID)
        processStatus(status, "toString");

    status = Impl_getRT(this->_impl, &rt);

    if (status != IMPL_VALID)
        processStatus(status, "toString");

    // set the char* to null
    ret_string[0] = '\0';

    sprintf(tmp_buffer, "orm=> %i\n", orm);
    strcat(ret_string, tmp_buffer);
    sprintf(tmp_buffer, "rt=> %i\n", rt);
    strcat(ret_string, tmp_buffer);

    return ret_string;
}

/// the coordinate equality operator
bool Coord3D_Celestiodetic::operator== (const Coord3D_Celestiodetic &rhs) const
{
    return (((*(SRF_Celestiodetic*)(this->_srf)) == (*(SRF_Celestiodetic*)(rhs._srf))) &&
            ARE_EQUAL(_values[0], rhs._values[0]) &&
            ARE_EQUAL(_values[1], rhs._values[1]) &&
            ARE_EQUAL(_values[2], rhs._values[2]));
}

/// the coordinate equality operator
bool CoordSurf_Celestiodetic::operator== (const CoordSurf_Celestiodetic &rhs) const
{
    return (((*(SRF_Celestiodetic*)(this->_srf)) == (*(SRF_Celestiodetic*)(rhs._srf))) &&
             ARE_EQUAL(_values[0], rhs._values[0]) &&
             ARE_EQUAL(_values[1], rhs._values[1]));
}
