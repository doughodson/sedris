 ...
 *    Date              Description
 *    ----              -----------
 *    06-11-1995        Original Code
 *    03-01-1997        Original Code
 *    08-05-2010        SRM edits - removed Get_UPS_Parameters() function,
 *                      since it isn't used by the SRM. Updated
 *                      Convert_Geodetic_To_UPS() parameter checks to
 *                      combine if statements for efficiency. Updated
 *                      Convert_UPS_To_Geodetic() hemisphere checks to
 *                      combine if statements for efficiency; similarly
 *                      for latitude checks.
 */

#include <math.h>    // Is needed to call the math functions
#include "polarst.h" // Is used to convert polar stereographic coordinates
#include "ups.h"     // Defines the function prototypes for the ups module
...
#define MAX_LAT    (PI * 0.5)    /* 90 degrees in radians */
#define MAX_ORIGIN_LAT (81.114528 * DEG_TO_RAD)
#define MIN_NORTH_LAT (83.5*DEG_TO_RAD)
#define MIN_SOUTH_LAT (-79.5*DEG_TO_RAD)
 ...
 */
  ...
  if (((Latitude < 0) && (Latitude > MIN_SOUTH_LAT))
   || ((Latitude >= 0) && (Latitude < MIN_NORTH_LAT)))
    Error_Code |= UPS_LAT_ERROR;
