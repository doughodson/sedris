...
const double PI_Over_4 = (PI * 0.25);
...
static double Polar_Origin_Lat = PI_OVER_2;           /* Latitude of origin in radians */
 ...
 */
  ...
  { /* no errors */
    ...
    if (fabs(fabs(Polar_Origin_Lat) - PI_OVER_2) > 1.0e-10)
    {
      ...
      tc = tan(PI_Over_4 - Polar_Origin_Lat * 0.5) / pow_es;
    }
    else
    {
      one_PLUS_es = 1.0 + es;
 ...
 */
  ...
  {  /* no errors */
    ...
    else
    {
      ...
      essin = es * slat;
      pow_es = POLAR_POW(essin);
      t = tan(PI_Over_4 - Latitude * 0.5) / pow_es;
